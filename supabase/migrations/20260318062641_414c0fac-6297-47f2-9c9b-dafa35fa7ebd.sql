...
-- Add "أرباح مستحقة" account to existing companies that have the equity root (code '3')
...
SELECT 
  ...
  false,
  ac.id,
  'أرباح مستحقة الدفع أو التوزيع'
FROM account_categories ac
WHERE ac.code = '3' AND ac.type = 'equity'
AND NOT EXISTS (
  SELECT 1 FROM account_categories ac2 
  WHERE ac2.company_id = ac.company_id AND ac2.code = '34'
);

-- Update create_default_accounts to include the new account
...
BEGIN
  ...
  (p_company_id, '34', 'أرباح مستحقة', 'equity', false, v_equity_id, 'أرباح مستحقة الدفع أو التوزيع');
