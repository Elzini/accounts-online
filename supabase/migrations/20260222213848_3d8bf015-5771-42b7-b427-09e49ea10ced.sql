...
-- Update trigger to also handle 'active' status for purchase invoices
...
BEGIN
  ...
  -- Sales uses 'issued', Purchases uses 'active'
  v_approved_status := CASE 
    WHEN NEW.invoice_type = 'purchase' THEN 'active'
    ELSE 'issued'
  END;
