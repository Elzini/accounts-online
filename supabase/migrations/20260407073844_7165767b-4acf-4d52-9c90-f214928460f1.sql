...
BEGIN
  ...
  FOR rec IN
    WITH RECURSIVE company_tables AS (
      SELECT c.table_name
    ...
    depth_calc AS (
      ...
      UNION ALL
      SELECT e.child_table, dc.depth + 1
      FROM depth_calc dc
      JOIN fk_edges e ON e.parent_table = dc.table_name
      WHERE e.child_table IN (SELECT table_name FROM company_tables)
        AND dc.depth < 50
    ),
    ordered AS (
      SELECT ct.table_name, COALESCE(MAX(dc.depth), 0) AS depth
      FROM company_tables ct
      LEFT JOIN depth_calc dc ON dc.table_name = ct.table_name
