...
-- Drop and recreate the initialize_company_defaults function with ALL settings
...
BEGIN
  ...
  -- ============================================
  -- 7. APPLY THEME SETTINGS (NEW)
  -- ============================================
  -- Primary color
  INSERT INTO public.app_settings (company_id, key, value)
  VALUES (NEW.id, 'primary_color', '#3b82f6')
  ON CONFLICT DO NOTHING;
  
  -- Sidebar background color
  INSERT INTO public.app_settings (company_id, key, value)
  VALUES (NEW.id, 'sidebar_bg_color', '#1f2937')
  ON CONFLICT DO NOTHING;
  
  -- Font family
  INSERT INTO public.app_settings (company_id, key, value)
  VALUES (NEW.id, 'font_family', 'Cairo')
  ON CONFLICT DO NOTHING;
  
  -- Font size
  INSERT INTO public.app_settings (company_id, key, value)
  VALUES (NEW.id, 'font_size', '16')
  ON CONFLICT DO NOTHING;
  
  -- ============================================
  -- 8. CREATE DEFAULT DASHBOARD CONFIG (NEW)
  -- ============================================
  ...
  -- ============================================
  -- 9. CREATE DEFAULT FISCAL YEAR (NEW)
  -- ============================================
...
-- Add default theme settings to default_company_settings if not exists
INSERT INTO public.default_company_settings (setting_key, setting_type, setting_value)
VALUES 
  ('primary_color', 'theme', '#3b82f6'),
  ('sidebar_bg_color', 'theme', '#1f2937'),
  ('font_family', 'theme', 'Cairo'),
  ('font_size', 'theme', '16')
ON CONFLICT DO NOTHING;

-- Add default dashboard config to default_company_settings if not exists
INSERT INTO public.default_company_settings (setting_key, setting_type, setting_value)
VALUES (
  'dashboard_config', 
  'dashboard', 
  '{"stat_cards": [{"id": "monthly_sales", "label": "مبيعات الشهر", "size": "medium", "enabled": true}, {"id": "monthly_purchases", "label": "مشتريات الشهر", "size": "medium", "enabled": true}, {"id": "available_cars", "label": "السيارات المتوفرة", "size": "medium", "enabled": true}], "layout_settings": {"cards_per_row": 4, "grid_gap": 4}}'
)
ON CONFLICT DO NOTHING;
