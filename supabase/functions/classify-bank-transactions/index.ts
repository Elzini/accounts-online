...
قواعد التصنيف:
- التحويلات للأشخاص = ذمم مدينة أو دائنة حسب السياق
- رسوم بنكية / عمولات = مصاريف بنكية
- Commodity Buy = مشتريات أو مخزون
- فواتير نظام سداد = مصاريف عامة أو مرافق
- رواتب = مصاريف رواتب
- إيجار = مصاريف إيجار
- إيرادات / مبيعات = حساب الإيرادات
- إذا لم تستطع التصنيف بدقة = استخدم حساب "حركات بنكية غير مصنفة" أو أقرب حساب مناسب

أجب بـ JSON array فقط. كل عنصر:
{"index": رقم, "account_id": "UUID الحساب", "account_code": "كود الحساب", "account_name": "اسم الحساب", "confidence": "high"|"medium"|"low", "reason": "سبب التصنيف"}`;

    const userPrompt = `شجرة الحسابات المتاحة:
${accountsList}

المعاملات البنكية المطلوب تصنيفها:
${txnSummary}

صنّف كل معاملة واربطها بالحساب المناسب من شجرة الحسابات أعلاه. أعد JSON array فقط.`;
