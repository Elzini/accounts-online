...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `أنت محلل فواتير مشتريات محترف. استخرج جميع البيانات من فاتورة المشتريات المقدمة.
أعد النتائج كـ JSON فقط بدون أي تنسيق markdown.`;

    const userContent: any[] = [
      {
        type: "text",
        text: `حلل فاتورة المشتريات التالية واستخرج جميع البيانات.
الملف: ${fileName}
...
${isPDF ? 'الملف مرفق كصورة/PDF.' : `المحتوى:\n${fileContent}`}`,
        ...
        tools: [
          {
            ...
            function: {
              ...
              description: "Extract structured data from a purchase invoice",
              parameters: {
                type: "object",
                properties: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "يرجى شحن رصيد الاستخدام" }), {
          status: 402,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
