...
serve(async (req) => {
  ...
  try {
    ...
    // Writable fields allowlist — sensitive/system columns (status, is_active, is_system,
    // company_id, created_by, inventory_number, managed_by) are intentionally excluded.
    const WRITABLE_FIELDS: Record<string, string[]> = {
      'customers': ['name', 'phone', 'email', 'address', 'credit_limit', 'tax_number', 'notes', 'contact_person', 'customer_type'],
      'suppliers': ['name', 'phone', 'email', 'address', 'tax_number', 'notes', 'contact_person', 'payment_terms'],
      'cars': ['name', 'chassis_number', 'plate_number', 'model', 'color', 'car_condition', 'purchase_price', 'purchase_date', 'supplier_id', 'payment_account_id'],
      'sales': ['car_id', 'customer_id', 'sale_price', 'sale_date', 'payment_method', 'notes'],
      ...
      'employees': ['name', 'job_title', 'department', 'phone', 'email', 'hire_date', 'notes'],
    };
