  ...
  return (
    <div ref={ref} className="bg-white max-w-[210mm] mx-auto text-black text-[11px]" style={{ fontFamily: 'Cairo, Arial, sans-serif' }} dir="rtl">
      ...
      <div className="text-center py-2 border-y-2 border-gray-800">
        <h2 className="text-lg font-bold">{invoiceType === 'purchase' ? 'فاتورة مشتريات' : L.invoiceTitle}</h2>
      ...
      <div className="px-4 py-2 border-b">
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px]">
          <div className="flex gap-1 border-b border-gray-200 pb-1">
            <span className="text-gray-500 w-20">{invoiceType === 'purchase' ? 'Supplier المورد' : `${L.buyerLabelEn} ${L.buyerLabel}`}</span>
          ...
          {data.plateNumber && (
            <div className="flex gap-1 col-span-2 border-b border-gray-200 pb-1">
              <span className="text-gray-500 w-20">{L.plateNumberLabelEn} {L.plateNumberLabel}</span>
      ...
      <table className="w-full text-[10px] border-collapse mx-0">
        <thead>
           <tr className="bg-gray-800 text-white">
             ...
             <th className="p-1.5 border border-gray-700 text-right">{L.descriptionColumn} Description</th>
             <th className="p-1.5 border border-gray-700 text-center w-12">{L.quantityColumn} Quantity</th>
             <th className="p-1.5 border border-gray-700 text-center w-16">{L.priceColumn} Price</th>
             <th className="p-1.5 border border-gray-700 text-center w-14">{L.taxColumn} VAT</th>
             <th className="p-1.5 border border-gray-700 text-center w-18">{L.totalColumn} Subtotal</th>
           </tr>
        </thead>
        <tbody>
          {itemsWithTax.map((item, i) => (
            <tr key={i} className="border-b">
              ...
              <td className="p-1.5 border text-center">{item.quantity}</td>
               <td className="p-1.5 border text-center">{item.unitPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
               <td className="p-1.5 border text-center">{item.taxRate}</td>
               <td className="p-1.5 border text-center font-medium">{item.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex border-t-2 border-gray-800">
        ...
        <div className="flex-1 text-[10px]">
           <div className="flex justify-between p-1.5 border-b"><span className="font-bold">Total {L.subtotalLabel}</span><span>{subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
           ...
           <div className="flex justify-between p-1.5 border-b"><span className="font-bold">{L.taxColumn} VAT</span><span>{taxAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
           <div className="flex justify-between p-1.5 border-b font-bold text-sm bg-gray-100"><span>Net {L.grandTotalLabel}</span><span>{total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
