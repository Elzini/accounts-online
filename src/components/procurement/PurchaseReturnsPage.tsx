...
import { Plus, Search, Trash2, Printer, Save } from 'lucide-react';
...
export function PurchaseReturnsPage() {
  ...
  const [items, setItems] = useState<ReturnItem[]>([{
    id: '1', description: '', quantity: 0, outgoingQty: 0, returnedQty: 0, unit: '',
    cost: 0, total: 0, vat: 0, grandTotal: 0,
  }]);
  ...
  const addItem = () => {
    setItems(prev => [...prev, {
      id: String(prev.length + 1), description: '', quantity: 0, outgoingQty: 0, returnedQty: 0,
      unit: '', cost: 0, total: 0, vat: 0, grandTotal: 0,
    }]);
  };
  ...
  }), { quantity: 0, total: 0, discount: 0, discountPercent: 0, vat: 0, grandTotal: 0 });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('credit_debit_notes').insert({
        ...
        note_date: form.returnDate, total_amount: totals.grandTotal,
        reason: form.notes || null, status: 'draft',
      });
      if (error) throw error;
    },
    onSuccess: () => {
          ...
          <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg border">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <Label className="w-28 text-xs shrink-0">{language === 'ar' ? 'رقم فاتورة الشراء' : 'Purchase Invoice #'}</Label>
                  <Input className="h-8 text-sm" value={form.invoiceNumber} onChange={e => setForm(p => ({ ...p, invoiceNumber: e.target.value }))} />
                </div>
                <div className="flex items-center gap-2">
                  <Label className="w-28 text-xs shrink-0">{language === 'ar' ? 'رقم المورد' : 'Supplier #'}</Label>
                  <Input className="h-8 text-sm" value={form.supplierNumber} onChange={e => setForm(p => ({ ...p, supplierNumber: e.target.value }))} />
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select value={form.taxType} onValueChange={v => setForm(p => ({ ...p, taxType: v }))}>
                    <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="purchase_return">{language === 'ar' ? 'مرتجع مشتريات' : 'Purchase Return'}</SelectItem>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    ...
                    <TableHead className="text-xs w-20">{language === 'ar' ? 'كمية صادرة' : 'Outgoing'}</TableHead>
                    ...
                    <TableHead className="text-xs w-20">{language === 'ar' ? 'التكلفة' : 'Cost'}</TableHead>
                ...
                <TableBody>
                  {items.map((item, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell><Input type="number" className="h-7 text-xs" value={item.outgoingQty || ''} onChange={e => updateItem(idx, 'outgoingQty', Number(e.target.value))} /></TableCell>
                      ...
                      <TableCell><Input type="number" className="h-7 text-xs" value={item.cost || ''} onChange={e => updateItem(idx, 'cost', Number(e.target.value))} /></TableCell>
            ...
            <div className="flex justify-between items-center p-3 bg-muted/30 rounded-lg border text-sm">
              ...
              <div className="flex gap-6">
                <span>{language === 'ar' ? 'الإجمالي' : 'Subtotal'}: <strong>{totals.total.toFixed(2)}</strong></span>
